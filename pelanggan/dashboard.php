...
$queueNoValue =
    $active_queue && $col_a_no && isset($active_queue[$col_a_no])
        ? $active_queue[$col_a_no]
        : null;
$queueNo =
    $queueNoValue !== null
        ? str_pad((string) $queueNoValue, 3, "0", STR_PAD_LEFT)
        : "---";
$queueStatus =
    $active_queue && $col_a_status && isset($active_queue[$col_a_status])
        ? strtoupper((string) $active_queue[$col_a_status])
        : "BELUM ADA";
$serviceName = $active_queue["nama_layanan"] ?? "Belum memilih layanan";
$barberName = $active_queue["nama_barber"] ?? "Barber tersedia";
$priceLabel = isset($active_queue["harga_layanan"])
    ? "Rp " . number_format((int) $active_queue["harga_layanan"], 0, ",", ".")
    : "Menunggu detail";
$waitEstimate = "0 min";
$shopLoad = 1;
...
if ($active_queue) {
    ...
    $cntQ = mysqli_query(
        $conn,
        "SELECT COUNT(*) as wait_count FROM antrian WHERE tanggal = '$qDate' AND no_antrian < $qNo AND LOWER(status_antrian) NOT IN ('selesai', 'dibatalkan')",
    );
    ...
    $activeCountQ = mysqli_query(
        $conn,
        "SELECT COUNT(*) as active_c FROM antrian WHERE tanggal = '$qDate' AND LOWER(status_antrian) NOT IN ('selesai', 'dibatalkan')",
    );
    ...
    if ($totalActive <= 2) {
        ...
        $shopLoad = 2;
    } elseif ($totalActive <= 6) {
        $shopLoad = 3;
    } elseif ($totalActive <= 10) {
        $shopLoad = 4;
    } else {
        $shopLoad = 5;
    }
    ...
    $qId = (int) $active_queue["id"];
    $trxCheck = mysqli_query(
        $conn,
        "SELECT id FROM transaksi WHERE antrian_id = '$qId' LIMIT 1",
    );
    $hasUnpaidTrx = mysqli_num_rows($trxCheck) === 0;
}
...
<body class="min-h-screen">
            ...
            <section class="grid grid-cols-1 gap-5 lg:grid-cols-12">
                <article class="customer-card relative overflow-hidden p-6 lg:col-span-8">
                    ...
                    <div class="flex flex-col justify-between gap-6 sm:flex-row sm:items-start">
                        ...
                        <div class="sm:text-right">
                            ...
                            <div class="mt-2 flex gap-1 sm:justify-end">
                                <?php for ($i = 1; $i <= 5; $i++): ?>
                                    <span class="h-7 w-2 <?= $i <= $shopLoad
                                        ? "bg-primary"
                                        : "bg-surface-high" ?>"></span>
                                <?php endfor; ?>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="mt-8 grid gap-5 md:grid-cols-3">
                        <div class="border border-outline bg-surface-panel p-5 text-center">
                            ...
                            <div class="mt-3 font-display text-5xl font-black text-primary"><?= htmlspecialchars(
                                $waitEstimate,
                            ) ?></div>
                            <p class="mt-2 text-sm text-on-muted"><?= $active_queue
                                ? "Antrean sedang berjalan"
                                : "Tidak ada antrean aktif" ?></p>
                        </div>
                        <div class="border border-outline bg-surface-panel p-5 text-center">
                            <p class="text-[11px] font-black uppercase tracking-[0.16em] text-on-muted">Posisi Anda</p>
                            <div class="mt-3 font-display text-5xl font-black text-on-surface">#<?= htmlspecialchars(
                                $queueNo,
                            ) ?></div>
                            <p class="mt-2 text-sm text-on-muted">Status: <?= htmlspecialchars(
                                $queueStatus,
                            ) ?></p>
                        </div>
                        <div class="border border-outline bg-surface-panel p-5 text-center">
                            ...
                            <p class="mt-3 font-bold text-on-surface"><?= htmlspecialchars(
                                $barberName,
                            ) ?></p>
                        </div>
                    </div>
                ...
                <article class="customer-card flex min-h-[300px] flex-col justify-between overflow-hidden bg-cover bg-center p-6 lg:col-span-4" style="background-image: linear-gradient(to bottom, rgba(18, 20, 20, 0.7), rgba(18, 20, 20, 0.96)), url('https://images.unsplash.com/photo-1605497788044-5a32c7078486?auto=format&fit=crop&w=900&q=80')">
                    <div>
                        ...
                        <h2 class="mt-5 font-display text-2xl font-black text-on-surface"><?= htmlspecialchars(
                            $serviceName,
                        ) ?></h2>
                        <p class="mt-3 text-sm leading-7 text-on-muted"><?= $active_queue
                            ? "Detail layanan aktif Anda siap dipantau dari dashboard ini."
                            : "Belum ada antrean aktif. Pilih layanan untuk mulai booking." ?></p>
                    </div>
                    <div class="mt-8">
                        <div class="mb-5 flex items-end justify-between gap-4">
                            <span class="font-display text-2xl font-black text-on-surface"><?= htmlspecialchars(
                                $priceLabel,
                            ) ?></span>
                            <span class="text-sm text-on-muted"><?= count(
                                $history_queues,
                            ) ?> kunjungan</span>
                        </div>
                        ...
                        <?php else: ?>
                            <span class="block w-full border border-primary bg-primary/10 py-3 text-center text-[12px] font-black uppercase tracking-[0.16em] text-primary">Pembayaran Selesai</span>
                        <?php endif; ?>
                    </div>
                </article>
                                ...
                                <?php if ($recentHistory): ?>
                                    <?php foreach ($recentHistory as $item): ?>
                                        ...
                                        <tr class="transition hover:bg-surface-panel">
                                            <td class="px-5 py-5 font-bold text-on-surface"><?= htmlspecialchars(
                                                $hDate,
                                            ) ?></td>
                                            <td class="px-5 py-5 text-on-muted"><?= htmlspecialchars(
                                                $item["nama_layanan"] ??
                                                    "Layanan",
                                            ) ?></td>
                                            <td class="px-5 py-5 text-on-muted"><?= htmlspecialchars(
                                            ...
                                            <td class="px-5 py-5 text-right font-bold text-primary"><?= htmlspecialchars(
                                                $hStatus,
                                            ) ?></td>
                                        </tr>
                                    <?php endforeach; ?>
                ...
                <article class="customer-card p-6 lg:col-span-4">
                    <div class="flex items-center gap-5">
                        ...
                        <div>
                            <h2 class="font-display text-2xl font-black text-on-surface"><?= htmlspecialchars(
                                $username,
                            ) ?></h2>
                    ...
                    <dl class="mt-7 space-y-4 text-sm">
                        <div class="flex justify-between border-b border-outline pb-3">
                            ...
                            <dd class="font-bold text-on-surface"><?= count(
                                $history_queues,
                            ) ?></dd>
                        </div>
                        <div class="flex justify-between border-b border-outline pb-3">
                            <dt class="text-on-muted">Antrean aktif</dt>
                            <dd class="font-bold text-on-surface"><?= $active_queue
                                ? "Ada"
                                : "Tidak ada" ?></dd>
                        </div>
                    </dl>
    ...
    <?php if ($active_queue) {
        pelanggan_payment_modal($active_queue);
    } ?>
    <?php if (
        isset($_GET["open_modal"])
    ): ?><script>window.addEventListener('DOMContentLoaded', () => { openBookingModal(); window.history.replaceState({}, document.title, window.location.pathname); });</script><?php endif; ?>
    <?php if (
        isset($_GET["open_payment_modal"])
    ): ?><script>window.addEventListener('DOMContentLoaded', () => { openPaymentModal(); window.history.replaceState({}, document.title, window.location.pathname); });</script><?php endif; ?>
</body>
</html>
