...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    $check = mysqli_query(
        $conn,
        "SELECT a.id, l.harga FROM antrian a JOIN layanan l ON a.layanan_id = l.id WHERE a.id = '$antrian_id' AND a.pelanggan_id = '$pelanggan_id' LIMIT 1",
    );
    $data = mysqli_fetch_assoc($check);

    if (!$data) {
        header(
            "Location: dashboard.php?status=error&msg=" .
                urlencode("Antrean tidak valid."),
        );
        exit();
    }

    $metode = isset($_POST["metode_pembayaran"])
        ? mysqli_real_escape_string($conn, $_POST["metode_pembayaran"])
        : "Tunai";
    ...
    if (
        ...
        $ext = strtolower(
            pathinfo($_FILES["bukti_pembayaran"]["name"], PATHINFO_EXTENSION),
        );
        ...
        if ($_FILES["bukti_pembayaran"]["size"] > 2 * 1024 * 1024) {
            ...
            if (
                !move_uploaded_file(
                    $tmp_name,
                    $folder_upload . $nama_file_bukti,
                )
            ) {
                $error_msg =
                    "Bukti pembayaran gagal disimpan. Silakan coba lagi.";
            }
        } else {
    ...
    if ($error_msg === "") {
        ...
        $existing = mysqli_query(
            $conn,
            "SELECT id FROM transaksi WHERE antrian_id = '$antrian_id' LIMIT 1",
        );
        $existingRow = $existing ? mysqli_fetch_assoc($existing) : null;

        if ($existingRow) {
            $transactionId = (int) $existingRow["id"];
            $buktiSql = $nama_file_bukti
                ? ", bukti_pembayaran = '" .
                    mysqli_real_escape_string($conn, $nama_file_bukti) .
                    "'"
                : "";
