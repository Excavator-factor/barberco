    ...
    <main data-pelanggan-main class="min-h-screen transition-[margin] duration-200 md:ml-64">
        ...
        <div class="mx-auto w-full max-w-[1440px] space-y-8 p-5 pb-28 md:p-8">
            <section class="flex flex-col justify-between gap-5 border-b border-outline pb-6 md:flex-row md:items-end">
                ...
                <span class="border border-outline bg-surface-panel px-4 py-2 text-sm font-bold text-on-muted"><?= count(
                    $history_queues,
                ) ?> data</span>
            </section>
                        ...
                        <tbody class="divide-y divide-outline text-sm">
                            <?php if (!empty($history_queues)): ?>
                                <?php foreach ($history_queues as $item): ?>
                                    ...
                                    $hDate =
                                        $col_a_tgl && isset($item[$col_a_tgl])
                                            ? date(
                                                "d M Y",
                                                strtotime($item[$col_a_tgl]),
                                            )
                                            : date("d M Y");
                                    $hStatus =
                                        $col_a_status &&
                                    ...
                                    $hDuration = isset($item["durasi_layanan"])
                                        ? (int) $item["durasi_layanan"] .
                                            " menit"
                                        : "Belum tersedia";
                                    $trxId = (int) ($item["transaksi_id"] ?? 0);
                                    ?>
                                    <tr class="transition hover:bg-surface-panel">
                                        <td class="px-5 py-5 font-display text-lg font-black text-primary">#<?= htmlspecialchars(
                                            $hNo,
                                        ) ?></td>
                                        ...
                                        <td class="px-5 py-5 text-right">
                                            <span class="<?= strtolower(
                                                $hStatus,
                                            ) === "selesai"
                                                ? "border-primary text-primary"
                                                : "border-outline text-on-muted" ?> inline-flex border px-3 py-1 text-[11px] font-black uppercase tracking-[0.16em]"><?= htmlspecialchars(
     $hStatus,
 ) ?></span>
                                        </td>
