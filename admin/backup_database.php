...
header('Content-Disposition: attachment; filename="barber_db-backup-' . date('Ymd-His') . '.sql"');
...
echo "CREATE DATABASE IF NOT EXISTS `" . str_replace('`', '``', $databaseName) . "`;\n";
...
while ($tableRow = $tables ? mysqli_fetch_row($tables) : null) {
    ...
    if ($rows && mysqli_num_rows($rows) > 0) {
        ...
        echo 'INSERT INTO ' . $safeTable . ' (' . implode(', ', $columns) . ") VALUES\n";
        $values = [];
