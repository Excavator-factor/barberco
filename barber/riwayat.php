        ...
        <div class="flex-1 overflow-y-auto custom-scrollbar">
            <div class="p-md md:p-lg max-w-7xl w-full mx-auto pb-24 md:pb-8">
                ...
                <div class="print-header mb-4">
                    ...
                    <p style="font-size:12px;color:#555;margin:0;">Kapster: <?= htmlspecialchars(
                        $barberName,
                    ) ?> &nbsp;|&nbsp; Periode: <?= htmlspecialchars(
     $filterLabel,
                ...
                <div class="mb-lg flex flex-col md:flex-row md:items-end justify-between gap-md border-b border-outline-variant pb-6 no-print">
                    <div>
                        ...
                        <p class="text-on-surface-variant text-sm">Daftar pelayanan yang telah Anda selesaikan — <span class="text-primary font-bold"><?= htmlspecialchars(
                            $filterLabel,
                        ) ?></span></p>
                    </div>
                    <div class="flex gap-4 flex-wrap">
                        <div class="bg-surface-container border border-outline-variant px-4 py-2 rounded-lg text-right">
                            ...
                            <p class="text-xl font-bold text-primary font-headline-md"><?= count(
                                $completedQueues,
                            ) ?></p>
                        </div>
                        <div class="bg-surface-container border border-outline-variant px-4 py-2 rounded-lg text-right">
                            <p class="text-xs text-on-surface-variant uppercase font-bold">Total Revenue</p>
                            <p class="text-xl font-bold text-primary font-headline-md">Rp <?= number_format(
                                $revenueToday,
                                0,
                                ",",
                                ".",
                ...
                <div class="print-summary" style="display:none;">
                    <div><strong>Total Sesi:</strong> <?= count(
                        $completedQueues,
                    ) ?></div>
                    <div><strong>Total Revenue:</strong> Rp <?= number_format(
                        $revenueToday,
                        0,
                        ",",
                        ".",
                ...
                <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 mb-4 no-print">
                    <div class="flex flex-wrap gap-2">
                        ...
                        <a href="?filter=<?= $fKey ?>"
                           class="filter-tab px-4 py-2 rounded-full border border-outline-variant text-xs font-bold uppercase tracking-widest <?= $isActive
                               ? "active"
                               : "" ?>">
                            <?= $fLabel ?>
                        </a>
                        ...
                        <table id="riwayatTable" class="w-full text-left print-table">
                            ...
                            <tbody>
                                <?php if ($completedQueues): ?>
                                    ...
                                    <tr class="border-b border-outline-variant hover:bg-surface-container-high/40 transition-colors">
                                        ...
                                        <td class="px-4 py-3">
                                            <p class="font-bold text-on-surface text-sm"><?= htmlspecialchars(
                                                $queue["nama_pelanggan"],
                                            ) ?></p>
                                        </td>
                                        <td class="px-4 py-3 text-sm text-on-surface-variant"><?= htmlspecialchars(
                                            $queue["nama_layanan"],
                                        ) ?></td>
                                        <td class="px-4 py-3 text-sm font-bold text-primary whitespace-nowrap">Rp <?= number_format(
                                            (int) $queue["harga"],
                                            0,
                                            ",",
                                            ".",
                                        ) ?></td>
                                        <td class="px-4 py-3 text-xs text-on-surface-variant whitespace-nowrap hidden sm:table-cell"><?= date(
                                            "d M Y",
                                            strtotime($queue["tanggal"]),
                                        ) ?></td>
                                        <td class="px-4 py-3 text-xs text-on-surface-variant whitespace-nowrap"><?= date(
                                            "H:i",
                                ...
                                <?php else: ?>
                                    <tr>
                                        <td colspan="7" class="px-4 py-12 text-center">
                                            ...
                                            <p class="text-sm text-on-surface-variant">Belum ada data untuk periode <strong><?= htmlspecialchars(
                                                $filterLabel,
                                            ) ?></strong>.</p>
                                        </td>
                                    </tr>
                            ...
                            <tfoot>
                                <tr class="bg-surface-container-low">
                                    <td colspan="3" class="px-4 py-3 text-xs font-bold uppercase tracking-widest text-on-surface-variant">Total (<?= count(
                                        $completedQueues,
                                    ) ?> sesi)</td>
                                    <td class="px-4 py-3 text-sm font-bold text-primary">Rp <?= number_format(
                                        $revenueToday,
                                        0,
                                        ",",
                                        ".",
