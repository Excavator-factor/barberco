...
<div class="flex h-screen overflow-hidden">
    ...
    <main class="flex-grow md:ml-[250px] flex flex-col h-screen overflow-hidden min-w-0">
        ...
        <div class="flex-1 overflow-y-auto custom-scrollbar">
            ...
            <?php if ($message): ?>
                <div class="mb-6 p-4 rounded-lg flex items-center justify-between border <?= $messageType === 'error' ? 'bg-error-container/20 border-error text-error' : 'bg-primary/10 border-primary text-primary' ?>">
            ...
            <div class="mb-lg flex flex-col md:flex-row md:items-end justify-between gap-md">
                <div>
                    <h2 class="font-headline-lg text-headline-lg mb-2 text-on-surface">Selamat datang kembali, <?= htmlspecialchars($barberName) ?>.</h2>
                    <p class="text-on-surface-variant max-w-xl">Stasiun Anda siap. Saat ini ada <span class="text-primary font-bold"><?= count($waitingQueues) ?> pelanggan</span> menunggu di antrean Anda hari ini.</p>
                </div>
                ...
                <form method="POST" action="dashboard.php" class="bg-surface-container-high p-1 rounded-xl flex items-center border border-outline-variant">
                    ...
                    <button type="submit" name="status" value="active" class="px-6 py-2 rounded-lg font-label-caps text-label-caps flex items-center gap-2 <?= $barberStatus === 'active' ? 'bg-primary text-on-primary active-glow' : 'text-on-surface-variant hover:text-on-surface' ?> transition-all duration-300">
                        <span class="w-2 h-2 rounded-full <?= $barberStatus === 'active' ? 'bg-on-primary animate-pulse' : 'bg-outline' ?>"></span>
                        AKTIF
                    </button>
                    <button type="submit" name="status" value="off_duty" class="px-6 py-2 rounded-lg font-label-caps text-label-caps flex items-center gap-2 <?= $barberStatus === 'off_duty' ? 'bg-error-container text-on-error-container' : 'text-on-surface-variant hover:text-on-surface' ?> transition-all duration-300">
                        <span class="w-2 h-2 rounded-full <?= $barberStatus === 'off_duty' ? 'bg-on-error animate-pulse' : 'bg-outline' ?>"></span>
                        TIDAK BERTUGAS
                    </button>
                </form>
            </div>
                    ...
                    <div class="p-md md:p-lg flex flex-col md:flex-row gap-md items-center">
                        <?php if ($activeQueue): ?>
                            ...
                            <div class="flex-grow text-center md:text-left">
                                <p class="font-label-caps text-label-caps text-primary uppercase tracking-widest mb-1">Layanan Saat Ini &middot; Antrean #<?= str_pad((string)$activeQueue['no_antrian'], 3, '0', STR_PAD_LEFT) ?></p>
                                <h3 class="font-headline-lg text-headline-lg mb-1 text-on-surface"><?= htmlspecialchars($activeQueue['nama_pelanggan']) ?></h3>
                                <p class="text-on-surface-variant mb-4 flex items-center justify-center md:justify-start gap-2">
                                    ...
                                    <?= htmlspecialchars($activeQueue['nama_layanan']) ?>
                                </p>
                                <div class="flex flex-wrap gap-2 justify-center md:justify-start">
                                    <span class="bg-surface-container-highest px-3 py-1 text-xs border border-outline-variant rounded text-on-surface">Dimulai <?= date('H:i', strtotime($activeQueue['waktu_dibuat'])) ?></span>
                                    <span class="bg-surface-container-highest px-3 py-1 text-xs border border-outline-variant rounded text-on-surface">Est. <?= (int)$activeQueue['durasi'] ?> mnt</span>
                                    <span class="bg-primary/10 text-primary border border-primary/30 px-3 py-1 text-xs rounded font-bold">Rp <?= number_format((int)$activeQueue['harga'], 0, ',', '.') ?></span>
                                </div>
                            </div>
                            <div class="flex flex-col gap-2 w-full md:w-auto">
                                <form method="POST" action="dashboard.php" onsubmit="return confirm('Selesaikan antrean ini?');">
                                    <input type="hidden" name="queue_id" value="<?= (int)$activeQueue['id'] ?>">
                        ...
                        <?php else: ?>
                            <div class="w-full py-8 text-center flex flex-col items-center">
                                ...
                                foreach ($waitingQueues as $wq) {
                                    if (empty($wq['barber_id']) || (int)$wq['barber_id'] === $barberId) {
                                        $nextQueue = $wq;
                                        break;
                                    }
                                }
                                ...
                                <?php if ($nextQueue): ?>
                                    <form method="POST" action="dashboard.php">
                                        <input type="hidden" name="queue_id" value="<?= (int)$nextQueue['id'] ?>">
                                        <button type="submit" name="action" value="start" class="bg-primary text-on-primary font-bold py-3 px-8 rounded-lg hover:scale-105 active:scale-95 transition-all flex items-center gap-2 shadow-lg">
                                            ...
                                            MULAI BERIKUTNYA (#<?= str_pad((string)$nextQueue['no_antrian'], 3, '0', STR_PAD_LEFT) ?> - <?= htmlspecialchars($nextQueue['nama_pelanggan']) ?>)
                                        </button>
                                    </form>
                                <?php else: ?>
                        ...
                        <div class="space-y-6">
                            <div class="flex justify-between items-end border-b border-outline-variant pb-2">
                                <div>
                                    ...
                                    <p class="text-2xl font-bold font-headline-md text-on-surface"><?= $stats['completed'] ?></p>
                                </div>
                            ...
                            <div class="flex justify-between items-end border-b border-outline-variant pb-2">
                                <div>
                                    ...
                                    <p class="text-2xl font-bold font-headline-md text-primary">Rp <?= number_format($revenueToday, 0, ',', '.') ?></p>
                                </div>
                                <div class="text-right">
                ...
                <div class="col-span-12 lg:col-span-7 bg-surface-container border border-outline-variant p-md rounded-xl">
                    <div class="flex justify-between items-center mb-md pb-4 border-b border-outline-variant">
                        ...
                        <div class="flex items-center gap-2 text-on-surface-variant text-sm">
                            ...
                            <span>Est. Tunggu: ~<?= count($waitingQueues) * 30 ?> mnt</span>
                        </div>
                    </div>
                    <div class="space-y-4">
                        <?php if ($waitingQueues): ?>
                            <?php foreach ($waitingQueues as $index => $q): 
                                $isOwnOrOpen = empty($q['barber_id']) || (int)$q['barber_id'] === $barberId;
                                $initials = strtoupper(substr($q['nama_pelanggan'], 0, 2));
                            ?>
                                <div class="flex items-center gap-4 p-4 rounded-lg bg-surface border border-outline-variant hover:border-primary transition-colors group">
                                    ...
                                    <div class="flex-grow min-w-0">
                                        <p class="font-bold text-on-surface truncate"><?= htmlspecialchars($q['nama_pelanggan']) ?></p>
                                        <p class="text-xs text-on-surface-variant uppercase truncate"><?= htmlspecialchars($q['nama_layanan']) ?> &middot; <?= (int)$q['durasi'] ?> mnt</p>
                                    </div>
                                    <div class="text-right flex-shrink-0">
                                        <p class="text-sm font-bold text-primary">#<?= str_pad((string)$q['no_antrian'], 3, '0', STR_PAD_LEFT) ?></p>
                                        <p class="text-[10px] text-outline uppercase"><?= $index === 0 ? 'Berikutnya' : 'Antrean #' . ($index + 1) ?></p>
                                    </div>
                        ...
                        <div class="flex gap-md">
                            <div class="w-20 h-20 rounded border border-outline-variant overflow-hidden flex-shrink-0 bg-surface-container-high flex items-center justify-center text-primary">
                                <?php if (!empty($_SESSION['avatar']) && file_exists(__DIR__ . '/../uploads/avatars/' . $_SESSION['avatar'])): ?>
                                    <img class="w-full h-full object-cover" alt="Master Barber Headshot" src="../uploads/avatars/<?= htmlspecialchars($_SESSION['avatar']) ?>"/>
                                <?php else: ?>
                                    <span class="material-symbols-outlined text-[40px]">person</span>
                                <?php endif; ?>
                            </div>
                            <div>
                                <h4 class="font-bold text-lg text-on-surface"><?= htmlspecialchars($barberName) ?></h4>
                        ...
                        <div class="grid grid-cols-2 gap-md pt-4 border-t border-outline-variant">
                            <div>
                                ...
                                <ul class="text-xs space-y-1 text-on-surface-variant">
                                    <?php 
                                    $specs = array_map('trim', explode(',', $barberSpecialties));
                                    foreach ($specs as $spec): if ($spec):
                                    ?>
                                        <li class="flex items-center gap-2"><span class="w-1.5 h-1.5 rounded-full bg-primary"></span> <?= htmlspecialchars($spec) ?></li>
